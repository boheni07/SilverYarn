...
| persona_summary | TEXT NULL | `GET /sync/download`의 `persona_snapshot.summary` 로컬 캐시(design.md §2.11 4단계 "단기 압축 기억"). *(v0.11 신규)* |
| persona_keywords | TEXT NULL | 위와 같은 응답의 `keywords`를 `"\|"`로 합쳐 저장 — Room `@TypeConverter` 없이 저장하는 트릭(`autobiography_fts`의 keywords join(" ")과 같은 이유, 컬럼 용도가 달라 구분자만 다름). *(v0.11 신규 — 아직 온디바이스 SLM(SlmEngine)이 스텁이라 소비하는 곳 없이 저장만 함)* |
...
| 0.11 | 2026-09-11 | design §2.11 4단계 "단기 압축 기억" 페르소나 스냅샷 서버측 생성(design.md v0.41) 후속 — §2.6 `device_state`에 `persona_summary`/`persona_keywords` 컬럼 신규. `GET /sync/download`의 `persona_snapshot`을 `SyncRunner.applyDownload()`가 그대로 저장(keywords는 `"\|"` join). 온디바이스 SLM(SlmEngine)이 아직 스텁이라 저장만 하고 소비하는 곳은 없음 | NUBiz AX Initiative |
