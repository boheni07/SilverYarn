...
| 58 | I1. "Zero External Data Egress" 사정거리 | **PII·원본 데이터(대화 원문·음성·사진 등)만 외부 유출을 절대 금지하고, 빌드툴·오픈소스 패키지 다운로드·웹폰트·상용 알림 SDK(FCM) 같은 "메타데이터 수준" 외부 통신은 허용**하는 것으로 확정(사용자 결정). 완전 에어갭은 채택하지 않음 — Q6의 FCM 유지 결정과 정합 | "PII 수준"의 경계를 CONVENTIONS.md 또는 별도 보안정책 문서에 명문화(무엇이 "PII 수준"이고 무엇이 "메타데이터 수준"인지 해석 분쟁 방지) |
