...
| 54 | Q3. 가족·복지사의 어르신 데이터 열람 근거 | **가족 = 제26조 위탁범위 내 이용(별도 동의 불필요), 복지사 = 제17조 제3자제공(전용 동의 필요)**으로 확정(사용자 결정, [#48](#) 갱신). 복지사는 서비스 계약의 직접 당사자가 아닌 시설·기관 소속 제3자로 봄 | `third_party_access` consent 유형 신설, 이 동의가 있는 사용자에 한해 `READ_ELDER_DATA_ROLES`에 social_worker 재포함(현재 fail-closed 해제 게이트) |
