...
| 59 | I2. B2G 시설 테넌시 착수 여부 | **지금 착수** — `organizations` 테이블 + 시설 경계 인가를 확정(사용자 결정). Q3에서 복지사를 제3자제공 동의 게이트로 열어두기로 한 것과 자연스럽게 이어지는 다음 단계 | `organizations` 테이블 + `family_members.org_id` 마이그레이션, 시설 경계 인가 술어(같은 org만 조회), apps/admin 시설 관리 화면 |
