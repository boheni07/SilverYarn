...
| 60 | I3. `PII_KEK` 키관리 인프라 이전 여부 | **환경변수 방식 유지**(Vault/OpenBao 인프라 구축은 보류), 단 **blind index 키만 지금 `PII_KEK`에서 정식 분리**하기로 확정(사용자 결정). 근거: 시크릿 매니저 인프라 자체가 선행돼야 하는 큰 작업이라 지금 범위 밖, 그러나 두 키가 같은 값에서 유도되는 것은 "하나 유출 시 전부 노출"이라는 즉시 해소 가능한 보안 결함 | 신규 환경변수 `BLIND_INDEX_KEY` 추가, `core/crypto.py` `blind_index()`가 `PII_KEK` 대신 이 키 사용하도록 교체, 기존 blind index 재계산·백필 |
