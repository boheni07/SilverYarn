...
| 61 | I4. 관측·에러추적 스택 구축 여부 | **지금 구축** — self-hosted GlitchTip(에러 추적) + Prometheus/Grafana/Loki(메트릭·로그)로 확정(사용자 결정). Sentry 등 SaaS는 I1의 PII 외부유출 금지 원칙상(에러 로그에 대화 내용이 섞일 수 있음) 사용 불가 | `infra/docker-compose.yml`에 GlitchTip·Prometheus·Grafana·Loki 추가(port 9670~9680 범위 내), 백엔드 에러 리포팅 SDK 연동, structlog→Loki 파이프라인 |
