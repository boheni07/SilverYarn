...
## 2.9 법무·인프라·경영 결정 대기 14건 일괄 확정 (Do 단계, 2026-09-12 — 사용자 결정)

> **⚠️ 성격 안내**: 아래 항목 중 Q1~Q6(법무)은 **정식 외부 법률 자문을 대체하지 않는다.** `docs/03-check/blocked-decisions-tracker.md`가 이 세션 내내 "⚖️ 법무 대기"로 분류해온 이유가 그대로 유효하다 — 이 표는 개발을 계속 진행하기 위해 서비스 운영자(사용자)가 채택한 **잠정 회사 정책**이며, 실제 법률 자문 결과가 다르게 나오면 재작업이 필요할 수 있다. I1~I5(인프라)·경영 3건은 순수 제품·운영 결정으로 이 제약이 적용되지 않는다.

| # | 항목 | 결정 | 후속 작업 |
|---|------|------|------|
| 52 | Q1. 정서점수의 개인정보보호법 제23조 민감정보 해당 여부 | **일반 개인정보로 취급 — 별도 명시 동의 없이 기존 온보딩 포괄동의로 충분**(사용자 결정, [#12](#) 대체). 정서 점수는 대화 원문의 파생 통계치이며 진료·심리상담 기록이 아니라는 해석. 정서 모니터링 파이프라인 자체를 켤지는 별개의 구현 착수 여부이며, 이 결정은 그 법적 전제만 정리한다 | 정서 write 경로(`emotion_scores`/`emotion_alerts`) 구현 착수 시 별도 동의 UI 불필요 |
| 53 | Q2. 성년후견 미개시 어르신에 대한 가족 대리동의 유효성 | **가족(배우자·자녀 등 1촌 이내) 대리동의를 유효로 인정**(사용자 결정, [#46](#) 갱신). CTO 권고 명시적 enum(self/proxy/legal_guardian)까지는 아니지만, self/proxy 대리동의 자체는 정식 법적 효력을 인정 | ✅ **구현 완료**(2026-09-12) — 백엔드 `POST /users/{id}/consent-logs`의 `granted_by`가 이미 이 경로를 지원하고 있었음(PR #6). 실제 필요했던 건 스키마가 아니라 **그걸 쓰는 화면**: `apps/web` `/consent`(신규) — 가족이 자신 명의로 대리 동의 부여/철회. `actor`(self/proxy)는 여전히 파생값으로 충분해 별도 enum 컬럼은 추가하지 않음(YAGNI로 판단) |
| 54 | Q3. 가족·복지사의 어르신 데이터 열람 근거 | **가족 = 제26조 위탁범위 내 이용(별도 동의 불필요), 복지사 = 제17조 제3자제공(전용 동의 필요)**으로 확정(사용자 결정, [#48](#) 갱신). 복지사는 서비스 계약의 직접 당사자가 아닌 시설·기관 소속 제3자로 봄 | ✅ **구현 완료**(2026-09-13) — `consent_type` enum에 `third_party_access` 추가(마이그레이션 0009). `auth_deps.authorize_elder_data_read()`(신규) — family/caregiver는 기존과 동일 무조건 허용, social_worker는 이 동의가 있을 때만 허용(`ConsentDirectory` Protocol + Fake로 단위 테스트). 챕터·대화·일정·사진 조회 10개 엔드포인트 전환. 실 인프라(Postgres+Keycloak) e2e로 동의 전 403→동의 후 200 왕복 검증(`e2e_keycloak_check.py`) |
| 55 | Q4. 정서점수 산출·통보의 의료기기법 규제대상 위험 | **"진단/판정" 표현을 전면 회피하고 참고용 지표로만 제공**하는 방향으로 확정(사용자 결정). "우울증 진단" 같은 의료적 판정 문구·알림 대신 "최근 대화 톤이 평소보다 적었어요" 같은 순수 통계·참고 문구만 사용 | 정서 기능 구현 시 카피·알림 문구 가이드라인에 반영(웹 대시보드·알림 문구 전부) |
| 56 | Q5. 원본음성·전사·벡터·사진·백업 보유기간·파기방법 | **crypto-shredding(키 삭제)을 적법한 파기 수단으로 채택**하고, **보유기간은 하드코딩 대신 DB 설정 테이블(`retention_policies`) + admin 콘솔 UI로 운영자가 직접 조정**하도록 확정(사용자 결정). CTO가 "가장 큰 단일 작업(2~3주)·온보딩 안정화 선결조건"으로 지목한 항목 | ✅ **구현 완료**(2026-09-13) — 구현 착수 전 schema.md 전체 FK `ON DELETE` 절을 재조사해 CTO의 "5개 저장소 통합 삭제" 프레이밍을 좁혔다: `users` 하위 거의 모든 테이블이 이미 `ON DELETE CASCADE`(**`user_encryption_keys` 포함**)라 `DELETE FROM users` 한 줄로 Postgres 쪽(크립토 슈레딩용 키 삭제 포함)은 이미 완결 — 실제 오케스트레이션이 필요한 건 Postgres 밖 **Qdrant·Neo4j·MinIO 3곳**뿐이었다. 마이그레이션 0012(`retention_policies`+`deletion_records` 신설, `conversation_chunks.retention_until`/`purged_at`). 신규 `retention` 모듈(정책 CRUD) + `RetentionPurgeService`(만료 청크 배치 파기, Qdrant/Neo4j 삭제 후 Postgres redaction) + `UserErasureService`(계정 전체 삭제, 외부 저장소 먼저→Postgres 마지막 순서로 재시도 안전성 확보) + `POST /users/{id}/erase` admin 전용 엔드포인트 + `worker.py` 신규 cron(매시 30분). apps/admin `/retention-policies`(보유일수 조정 화면) + 사용자 목록 "계정 삭제" 2단계 확인 danger-zone. 시간 기반 보유기간은 **의도적으로 conversation_chunks 원문 하나로만 스코프를 좁힘**(원본음성은 이미 즉시삭제 #30, 챕터·사진은 자서전 결과물이라 계정 존속기간 보관) — "N일 비활성 시 계정 자동삭제" 같은 자율 파괴적 동작은 채택하지 않음(안전 설계 판단). 실 Docker 인프라(Postgres+Qdrant+Neo4j+MinIO)로 보유기간 계산→만료→파기→redaction, 계정 삭제→cascade+외부저장소 정리→감사로그 전 과정 왕복 검증. **부수 발견·수정**: `VECTORDB_API_KEY=""`(빈 문자열)가 qdrant-client의 `api_key is not None` 판정에 걸려 로컬 평문 HTTP Qdrant에 https로 접속을 시도하던 잠재 버그(`VectorDBClient`) 발견 및 수정 — 실 인프라 검증 중 실제 호출이 SSL 에러로 실패해 발견. 이 결정으로 **법무·인프라·경영 미결 14건이 전부 구현 완료** |
| 57 | Q6. FCM/SMS/이메일·Google Fonts의 국외이전 고지·동의 대상 여부 | **FCM(구글, 미국 서버) 유지 + 온보딩 동의 화면에 국외이전 고지·동의 UI 추가**로 확정(사용자 결정). 벤더 교체보다 고지·동의 UX를 추가하는 쪽 선택 | 🔨 **부분 구현**(2026-09-13) — `consent_type` enum에 `international_transfer` 추가(마이그레이션 0010). 모바일 온보딩 동의 화면에 `data_collection`과 구분되는 별도 체크박스(선택, 기본 미동의) + 고지 문구 추가, `OnboardingCoordinator`가 두 동의를 모두 기록. **남은 것**: 알림 발송 어댑터(FCM/SMS/이메일) 자체는 여전히 미구현 — 이번 라운드는 그 구현에 앞서 동의 이력만 확보 |
| 58 | I1. "Zero External Data Egress" 사정거리 | **PII·원본 데이터(대화 원문·음성·사진 등)만 외부 유출을 절대 금지하고, 빌드툴·오픈소스 패키지 다운로드·웹폰트·상용 알림 SDK(FCM) 같은 "메타데이터 수준" 외부 통신은 허용**하는 것으로 확정(사용자 결정). 완전 에어갭은 채택하지 않음 — Q6의 FCM 유지 결정과 정합 | ✅ **구현 완료**(2026-09-13) — [`data-classification-policy.md`](../../02-design/data-classification-policy.md) 신규 작성. schema.md 실제 컬럼을 Tier 1(PII 원본데이터, 암호화+외부반출 절대금지)~Tier 4(비개인정보, 외부통신 허용)로 분류하고 신규 컬럼 추가 시 판단 체크리스트 포함 |
| 59 | I2. B2G 시설 테넌시 착수 여부 | **지금 착수** — `organizations` 테이블 + 시설 경계 인가를 확정(사용자 결정). Q3에서 복지사를 제3자제공 동의 게이트로 열어두기로 한 것과 자연스럽게 이어지는 다음 단계. **접근권 부여 방식은 안전망(defense-in-depth)으로 확정**(2026-09-13 사용자 결정) — 기존 1:1 `family_members` 연결(admin 중개 초대, decisions #51)을 그대로 1차 권한 경로로 유지하고, `organizations`는 그 위에 "시설이 다르면 개별 연결이 있어도 차단"하는 2차 안전망으로만 쓴다. 조직원이 조직 소속 어르신 전체를 자동 조회하는 정식 B2G 대량 권한 부여 모델(더 실전 운영에 가까움)은 이번 라운드 범위 밖 — 정식 B2G 운영모델 확정 후 재검토 | ✅ **구현 완료**(2026-09-13) — 마이그레이션 0011(`organizations` 테이블 + `users`/`family_members`/`invitations`.`org_id`). 신규 `organizations` 모듈(4계층). `auth_deps.authorize_elder_data_read()`에 테넌시 체크 추가(`UserDirectory`+`ElderAccessContext`로 리팩터링, 기존 `ConsentDirectory` 단독 파라미터를 흡수). apps/admin `/organizations`(시설 등록·목록) + family-members 화면 시설 배정/초대 시 시설 선택. 유닛테스트 19건, 실 인프라 시나리오 검증(같은 시설 허용/다른 시설 차단) |
| 60 | I3. `PII_KEK` 키관리 인프라 이전 여부 | **환경변수 방식 유지**(Vault/OpenBao 인프라 구축은 보류), 단 **blind index 키만 지금 `PII_KEK`에서 정식 분리**하기로 확정(사용자 결정). 근거: 시크릿 매니저 인프라 자체가 선행돼야 하는 큰 작업이라 지금 범위 밖, 그러나 두 키가 같은 값에서 유도되는 것은 "하나 유출 시 전부 노출"이라는 즉시 해소 가능한 보안 결함 | ✅ **구현 완료**(2026-09-12) — `BLIND_INDEX_KEY` 환경변수 신설, `PiiCrypto.__init__(bidx_key=)` + `from_settings` 교체(하위호환 폴백 유지), `scripts/backfill_blind_index.py` 신규. design.md v0.45 |
| 61 | I4. 관측·에러추적 스택 구축 여부 | **지금 구축** — self-hosted GlitchTip(에러 추적) + Prometheus/Grafana/Loki(메트릭·로그)로 확정(사용자 결정). Sentry 등 SaaS는 I1의 PII 외부유출 금지 원칙상(에러 로그에 대화 내용이 섞일 수 있음) 사용 불가 | ✅ **구현 완료**(2026-09-13) — `infra/docker-compose.yml`에 GlitchTip(db+redis+web all_in_one+**bootstrap 자동화**)·Prometheus·Loki·**Grafana Alloy**(Promtail 2026-03-02 EOL로 대체)·Grafana 추가. 호스트 포트는 9679(GlitchTip)·9680(Grafana)만 사용(9670-9680 전 슬롯 소진). `core/observability.py`(sentry-sdk 에러 캡처, `OBS_GLITCHTIP_DSN` 없으면 꺼짐 + prometheus-fastapi-instrumentator `/metrics`), `core/logging.py`(실제 JSON 포맷으로 정정 — 이전엔 "구조화 로그"라 문서화했지만 실제로는 일반 텍스트였던 코드·문서 드리프트 발견). **부수 발견·수정**: `handle_unexpected`(core/errors.py)가 예외를 로깅도 캡처도 없이 완전히 삼키고 있던 버그 — 이번에 로깅+`sentry_sdk.capture_exception` 추가. 실 인프라로 3개 파이프라인 전부 왕복 검증(메트릭 스크레이프 `up=1`, JSON 로그→Alloy→Loki 쿼리 확인, 테스트 예외→GlitchTip Issue 생성 확인) |
| 62 | I5. 서버 LLM(vLLM)의 실시간 대화 관여 여부 | **서버 LLM은 챕터 생성·Compaction·Critic Agent 등 비실시간 작업 전용으로 한정, 실시간 말벗돌봄 대화는 온디바이스 SLM만 담당**하는 것으로 확정(사용자 결정). 기획서의 "오프라인 우선" 원칙과 정합 — Wi-Fi 미접속 상태에서도 말벗돌봄이 동작해야 함. GPU VRAM 예산표 자체는 온디바이스 SLM 실기기 벤치마크([#27](#)) 결과가 나와야 확정 가능해 **여전히 보류** | 벤치마크(`docs/03-check/ondevice-slm-benchmark-protocol.md`) 완료 후 VRAM 예산표·admission control 확정 |
| 63 | Phase 1~3 착수 일정·예산·인력 (구 #2/#14) | 구체 예산·인력은 계속 미결이나, **"3개월 내 소규모 파일럿(가족 몇 명·어르신 몇 분) 출시"를 대략적 목표로 확정**(사용자 결정). 정식 예산 승인과 무관하게 진행 가능한 항목 위주로 이 목표에 맞춰 우선순위를 재조정 | 파일럿 범위에 맞춰 남은 후속 작업(§2.9 전체) 중 우선순위 선별 |
| 64 | 결제 도메인 — B2C/B2G 과금 방식 (구 #18/#23) | **파일럿 기간은 무료·수동결제(계좌이체 등 오프라인)로 진행, 결제 도메인 자체는 계속 Phase 1 스코프 아웃 유지**로 확정(사용자 결정). #63의 3개월 파일럿 목표에 결제 자동화가 불필요하다는 판단 | 없음(스코프 아웃 유지) — 정식 베타·B2G 확장 시점에 재논의 |
| 65 | 외부 고품질 TTS 벤더·비용·DPA (구 #13) | 기획서 원안대로 **Phase 3(외부연계 시점)로 계속 유예**하는 것으로 재확인(사용자 결정). 파일럿은 온디바이스 네이티브 TTS(`AndroidNativeTtsEngine`, 이미 구현됨)만으로 충분 | 없음(Phase 3까지 보류 유지) |

> **적용 매핑**: #52=Q1, #53=Q2, #54=Q3, #55=Q4, #56=Q5, #57=Q6, #58=I1, #59=I2, #60=I3, #61=I4, #62=I5, #63=구#2/#14, #64=구#18/#23, #65=구#13. 원본 §3 표의 #12·#46·#48과 CTO 원문 질문 Q1~Q6·I1~I5는 [`cto-review-2026-09-05.md`](../../02-design/cto-review-2026-09-05.md) §2·§3, [`blocked-decisions-tracker.md`](../../03-check/blocked-decisions-tracker.md)에 원문이 있다.
...
| # | 항목 | 사유 | 상태 |
|---|------|------|:---:|
| 12 | 정서 모니터링 알림의 법적/윤리적 기준 (누구에게, 어떤 임계치로 통보할지) | 어르신 본인 동의 없이 제3자(가족·복지사)에게 정신건강 관련 정보가 전달될 수 있는 사안으로, 개인정보보호법·의료법 등 법적 책임이 따름. AI가 임의로 기준을 정하면 실제 서비스에서 법적 리스크가 발생할 수 있어 **반드시 법무·윤리 검토를 거쳐야 함** | ⚖️ 알림 임계치는 여전히 별도 검토 필요 — 단, 민감정보 해당 여부는 [#52](#) 확정 |
| 13 | 외부 TTS 연계의 구체 대상 서비스·비용·DPA 체결 | 계약·비용 문제로 경영진/법무 결정 사항 (§1의 결정에 따라 Phase 3 착수 시점으로 유예) | ✅ [#65](#) 확정 (Phase 3 유예 재확인) |
| 14 | Phase 1~3 착수 일정·예산·인력 | 경영진 승인 사항 (§1 참조) | ✅ [#63](#) 확정 (대략적 목표: 3개월 내 파일럿) |
| 23 | 구독·결제 PG사·요금제 | #18에서 Phase 1 스코프 아웃은 확정했으나, 실제 결제 도메인 설계 자체는 B2C/B2G 과금 방식이 정해져야 가능 (경영/법무 결정 사항) | ✅ [#64](#) 확정 (스코프 아웃 유지, 파일럿은 무료·수동결제) |
| 46 | 가족 대리동의의 법적 근거 + 정보주체(어르신) 권리행사 모델 | CTO 검토 [B2](../../02-design/cto-review-2026-09-05.md) — 성년후견 미개시 어르신에 대한 가족 대리동의 유효성·본인동의 필수범위가 법무 확정 대기. **Do 단계 임시 구현(2026-09-09)**: `consent_logs.granted_by` 유무로 `actor`(self/proxy)를 파생만 하고, CTO 권고인 명시적 enum(self/proxy/**legal_guardian**)·`data_subject` RBAC 행은 도입하지 않음 — `legal_guardian`을 코드가 임의 정의하면 안 되므로. 법무 회신 후 스키마 컬럼(`actor` enum)·RBAC 반영 | ✅ [#53](#) 확정 (가족 대리동의 유효 인정, 잠정 정책) |
| 48 | 복지사(social_worker)의 어르신 데이터 열람 근거 | CTO 검토 B2 — "가족·복지사 열람이 제17조 제3자제공인가, 제26조 위탁범위 내 이용인가". 이걸 표현할 consent 유형(`third_party_access` 등)·범위가 법무 확정 대기. **Do 단계 임시 구현(2026-09-09, 0.16)**: `READ_ELDER_DATA_ROLES`에서 social_worker 제외해 **fail-closed**(챕터·사진·대화·일정 조회 차단, 정서 알림·본인 알림설정은 유지). 법무 확정 후 전용 consent 유형 신설 + 그 동의 상태를 게이트로 복지사 재포함 | ✅ [#54](#) 확정 (복지사=제3자제공, `third_party_access` 동의 게이트) |

> 12번 항목에 대해 초안 성격의 시작점을 제안할 수는 있으나(예: 1차 알림은 가족에게만, 복지사는 가족 동의 시에만, 무응답 시 단계적 에스컬레이션), **이를 정식 기준으로 채택하는 것은 법무·윤리 검토 이후에만 가능**하다.

> **2026-09-12 갱신**: 아래 §2.9에서 #12·#13·#14·#23·#46·#48을 포함한 `blocked-decisions-tracker.md`의 법무·인프라·경영 미결 14건 전체를 **사용자가 임시 회사 정책으로 일괄 확정**했다(정식 외부 법률 자문 대체 아님 — §2.9 서두 참조). 이 표의 상태는 참고용으로 남겨두고, 실제 최신 상태는 §2.9를 따른다.
