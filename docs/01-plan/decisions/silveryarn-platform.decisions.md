...
| 57 | Q6. FCM/SMS/이메일·Google Fonts의 국외이전 고지·동의 대상 여부 | **FCM(구글, 미국 서버) 유지 + 온보딩 동의 화면에 국외이전 고지·동의 UI 추가**로 확정(사용자 결정). 벤더 교체보다 고지·동의 UX를 추가하는 쪽 선택 | 알림 발송 어댑터(FCM/SMS/이메일) 구현 + 온보딩 동의 화면에 "푸시알림 전송을 위해 구글(미국)에 기기토큰이 이전됩니다" 고지·동의 항목 추가 |
