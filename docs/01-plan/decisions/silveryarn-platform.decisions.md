...
| 56 | Q5. 원본음성·전사·벡터·사진·백업 보유기간·파기방법 | **crypto-shredding(키 삭제)을 적법한 파기 수단으로 채택**하고, **보유기간은 하드코딩 대신 DB 설정 테이블(`retention_policies`) + admin 콘솔 UI로 운영자가 직접 조정**하도록 확정(사용자 결정). CTO가 "가장 큰 단일 작업(2~3주)·온보딩 안정화 선결조건"으로 지목한 항목 | `retention_policies` 테이블(데이터종류·보유일수) 마이그레이션, `*.retention_until`/`*.purged_at` 컬럼, apps/admin 보유기간 설정 화면, 5개 저장소(PostgreSQL·MinIO·Qdrant·Neo4j·모바일 Room) crypto-shredding 오케스트레이터(arq cron) |
