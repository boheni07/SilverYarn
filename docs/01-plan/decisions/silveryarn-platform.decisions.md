...
| 53 | Q2. 성년후견 미개시 어르신에 대한 가족 대리동의 유효성 | **가족(배우자·자녀 등 1촌 이내) 대리동의를 유효로 인정**(사용자 결정, [#46](#) 갱신). CTO 권고 명시적 enum(self/proxy/legal_guardian)까지는 아니지만, self/proxy 대리동의 자체는 정식 법적 효력을 인정 | `consent_logs.actor` enum 컬럼(self/proxy) 마이그레이션, 온보딩 동의 화면 대리동의 분기 UI |
