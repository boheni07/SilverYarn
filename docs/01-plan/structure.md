...
│       │   ├── modules/             # 도메인 모듈 — 상세는 §2. 12개 논리 모듈 전부 구현
...
├── packages/
│   ├── web-shared/              # ★ apps/web·apps/admin 공유 코드 (@silveryarn/web-shared) — Keycloak 인증(auth.ts·proxy·auth-route), API 클라이언트(api/), UI 프리미티브(ui/), AppHeader. TS 소스 노출 + 각 앱 next.config `transpilePackages`
│   ├── py-common/                # (미스캐폴딩) 서버 공통 유틸 — 현재는 core_service/core·shared로 충분
│   └── design-tokens/            # (미스캐폴딩) 디자인 토큰 단일소스 — 현재는 각 앱 globals.css
...
| 1.36 | 2026-09-11 | **npm workspaces 도입 + `packages/web-shared` 추출**(decisions #50). 루트 `package.json`(`workspaces: apps/web·apps/admin·packages/*`), 루트 단일 `package-lock.json`. `@silveryarn/web-shared`: Keycloak 인증(`auth.ts`·`proxy`·`auth-route` — PR #19에서 cp됐던 보안 코드), API 클라이언트(`api/`, server-only), UI 프리미티브(`ui/`), `AppHeader`(`brand` prop). 두 앱은 얇은 shim(`src/proxy.ts`·`route.ts`·`components/ui/*`·`services/errors.ts`) + 나머지는 `@silveryarn/web-shared/*` 직접 import. `types/*`는 앱별 유지. eslint 계층 경계 `import/no-restricted-paths`→`no-restricted-imports`(`@silveryarn/web-shared/api`). `next.config.ts`에 `transpilePackages`. CI web·admin job은 워크스페이스 루트에서 `npm ci`. §3(트리)에 `packages/web-shared` 반영 | NUBiz AX Initiative |
| 1.35 | 2026-09-11 | PDCA Check #2(`docs/03-check/gap-analysis-2026-09-11.md`) 반영 — 트리 주석·§2 "Phase 1 구현 범위"의 모듈 수를 "8개/10개"에서 **12개**로 정정(`consent`·`notifications` 누락돼 있었음). `author`가 §2.11 Compaction Engine·Critic Agent도 포함함을 명시. `photos` orphan cleanup 배치 "아직 미구현" → arq cron job(`worker.py`)으로 구현됨으로 정정 | NUBiz AX Initiative |
| 1.34 | 2026-09-11 | apps/admin Keycloak 로그인 연동(decisions #49) — apps/web과 동일 파일 구성(`auth.ts`·`proxy.ts`·`api/auth/[...nextauth]`·`login/page.tsx`·`AppHeader.tsx`·`next-auth.d.ts` 이식). admin은 GET 전용이라 Server Action 없음 — `lib/api/client.ts`에 `server-only` + `auth()` 토큰 주입, 3개 서비스의 `authToken: "dev"` 제거. `package.json` dev 스크립트 `next dev -p 3001`(web 3000과 공존). realm `silveryarn-web` `redirectUris`에 `localhost:3001/*` 추가. 브라우저 flow 검증(admin family_member로 `require_roles(admin)` 통과) | NUBiz AX Initiative |
| 1.33 | 2026-09-11 | apps/web Keycloak 로그인 연동(decisions #49) — Auth.js/NextAuth v5 + Keycloak provider(public client, PKCE). 신규: `src/auth.ts`(리프레시 포함), `src/app/api/auth/[...nextauth]/route.ts`, `src/proxy.ts`(Next 16 middleware rename), `src/app/login/page.tsx`, `src/components/AppHeader.tsx`(로그아웃), `src/types/next-auth.d.ts`, `src/lib/upload.ts`(MinIO PUT을 server-only 밖으로 분리), `features/*/actions.ts` 4개(Server Action). `lib/api/client.ts`에 `import "server-only"` + `auth()` 토큰 주입 — 서비스의 `authToken: "dev"` 전부 제거. 클라이언트 컴포넌트 5개는 Server Action 호출로 전환. `next-auth@5` 의존성, `.env.example`에 AUTH_* 추가. 로컬 실 flow 브라우저 검증(로그인→세션 쿠키→실 Bearer로 조회·PUT→Postgres 반영). **apps/admin은 미이식** | NUBiz AX Initiative |
| 1.32 | 2026-09-11 | apps/web `(family)/notification-settings` 화면 신규 — `notifications` 모듈(PR #3)의 첫 웹 소비자. `types/notification-setting.ts`(구 `consent-log.ts`의 스테일 `NotificationSetting` 인터페이스를 실 백엔드 응답 형태로 대체·이동), `services/notification-settings.ts`(get/replace), `features/notification-settings/NotificationSettingsForm.tsx`(client component, 3채널×3항목 그리드, PUT 전체 교체). lint/type-check/build 통과. §5.1 화면 인벤토리에 전 화면 구현 상태 표기 | NUBiz AX Initiative |
| 1.31 | 2026-09-10 | Critic Agent(design §2.11 3단계) 구현 — `questions` 큐에 **생성 경로** 신설(이전엔 읽기 전용). `core/clients/llm_client.py`에 `critique_and_generate_questions`(vLLM, 4축), `author` 도메인에 `GeneratedQuestion`, `QuestionRepository`에 `count_unanswered_by_user`/`create_many`, `QuestionService.generate_followups`(큐 8개 상한·중복 제거), 업로드 파이프라인이 챕터 갱신 후 best-effort 호출(`question_service`를 파이프라인·worker.py에 배선). 유닛테스트 8건(155개), 실 Postgres로 삽입·카운트 검증 | NUBiz AX Initiative |
