...
| id | UUID | Y | PK |
| user_id | UUID | Y | FK → users.id |
| consent_type | enum(`data_collection`,`external_tts_optin`,`external_llm_optin`,`third_party_access`,`international_transfer`) | Y | 동의 유형. `third_party_access`는 v1.14(마이그레이션 0009, decisions.md #54) — 복지사(social_worker)의 어르신 데이터 열람 전용 동의. `international_transfer`는 v1.15(마이그레이션 0010, decisions.md #57) — FCM 푸시알림 발송 시 기기토큰의 국외이전(구글, 미국) 고지·동의 |
...
-- v1.14: 'third_party_access' 추가(마이그레이션 0009, decisions.md #54). v1.15:
-- 'international_transfer' 추가(마이그레이션 0010, decisions.md #57) — DDL은 최신
-- 상태 기준, 실제 배포는 ALTER TYPE ... ADD VALUE로 증분 적용(Postgres는 enum 값
-- 삽입/생성 시점 변경을 지원하지 않는다).
CREATE TYPE consent_type AS ENUM ('data_collection', 'external_tts_optin', 'external_llm_optin', 'third_party_access', 'international_transfer');
