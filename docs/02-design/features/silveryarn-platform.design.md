...
>
> **구현 상태 (v0.28)**: 앱 시작 게이트 완성. `presentation/AppEntry.kt`가 `device_state` 조회로 시작 목적지를 정한다 — `device_id`가 있으면(온보딩 3연쇄 완료분) **온보딩·최초 동기화를 건너뛰고 바로 홈**, 없으면 온보딩 → **최초 Wi-Fi 동기화 화면**(`presentation/sync/FirstSyncScreen.kt` — 위 흐름의 "최초 Wi-Fi 동기화" 단계, `sync/SyncRunner.kt`를 1회 실행해 자서전 스냅샷·질문 큐·일정을 로컬 캐시에 채움, 실패 시 "건너뛰기"로 오프라인 진입 가능) → 홈. `install_mode` 분기(§9.3): `MainActivity`가 확정 모드를 받아 `installmode/KioskController.kt`로 `kiosk`면 lockTask 진입(decisions.md #6 — Device Owner가 아니면 조용히 무시). 동기화 절차 본체는 `SyncWorker.doWork()`에서 `SyncRunner`로 분리(Worker·최초 동기화 화면 공유). **"가족 계정 웹 콘솔 초대" 단계는 이 앱 흐름에 미포함** — 갓 온보딩한 어르신 기기(Device Token)는 `POST /invitations`(2FA + family role) 호출 권한이 없다. **v0.42: admin 중개 경로로 확정**(웹 콘솔/admin 경로 vs device-token 부트스트랩 두 안 중 admin 선택, 인가모델 변경 없이 기존 `is_admin` 우회만 재사용) — apps/admin `(admin)/family-members` 화면(신규, admin의 첫 쓰기 화면)에서 운영자가 `POST /invitations`를 호출해 초대 링크를 만들고, apps/web `/invitations/[token]`(신규)에서 초대받은 가족이 로그인 후 수락한다. **모바일 앱은 변경 없음** — 이 경로는 전적으로 웹 콘솔/admin 쪽에서 해결된다.
>
> **구현 상태 (v0.47, [decisions.md #57](../../01-plan/decisions/silveryarn-platform.decisions.md) Q6, 2026-09-13)**: 동의 단계에 **국외이전 동의(선택)** 항목을 추가했다 — FCM(구글, 미국) 유지 결정에 따라 푸시알림 발송 시 기기토큰이 국외로 이전되는 것을 고지하고, `data_collection`과 구분되는 별도 동의로 받는다(체크박스, 기본 미동의). `OnboardingCoordinator`가 `international_transfer` consent를 동의/미동의 값 그대로 기록(둘 다 이력이 남아야 함). 알림 발송 채널(FCM 어댑터) 자체는 미구현 — 동의 UI가 실제 구현보다 앞서는 것은 이 프로젝트의 반복되는 패턴(정서 모니터링 opt-out 기본값과 동일).
...
| GET | /api/v1/me | 로그인 세션이 어느 어르신(들)에 연결됐는지 자기참조 조회(신규 v0.58) — 새 조회 로직 없이 `require_auth`가 이미 계산한 `AuthContext.memberships`를 그대로 직렬화. apps/web이 예전에 화면마다 `?userId=`를 사람이 직접 입력받던 것(screen-definitions.md §0.3 "임시방편")을 이 엔드포인트로 대체(`services/me.ts` `resolveCurrentUserId`/`resolveCurrentMembership`) | Bearer(`require_auth`) — 특정 어르신을 대상으로 하지 않아 `authorize_elder_data_read` 류의 2FA 게이트는 없음, 멤버십 0건이면 403 |
...
| GET | /api/v1/organizations | B2G 시설 목록 조회(신규 v0.49, decisions.md #59, I2) | Admin |
...
- **B2G 시설 테넌시 안전망 (v0.50, [decisions.md #59](../../01-plan/decisions/silveryarn-platform.decisions.md), I2)**: `authorize_elder_data_read()`가 social_worker 게이트 다음으로 이 체크도 한다 — caregiver/social_worker의 `family_members.org_id`가 채워져 있으면(시설 소속), 그 어르신(`users.org_id`)도 같은 시설이어야 통과한다. B2C 개인(org_id 없음)은 이 체크 자체를 안 탄다. **정식 B2G 대량 권한 부여 모델(조직원이 조직 소속 어르신 전체를 자동 조회)은 스코프 밖** — 기존 1:1 `family_members` 연결(admin 중개 초대, decisions #51)이 여전히 1차 권한 부여 경로이고, org_id는 "설정 실수로 다른 시설 직원이 잘못 연결된 경우"의 2차 안전망이다. 어르신 시설 배정은 apps/admin `PUT /users/{id}/organization`(신규), 직원 시설 소속은 초대 시점(`invitations.org_id` → 수락 시 `family_members.org_id`로 이전) 결정.
...
| 0.42 | 2026-09-11 | Do 단계 — 첫 가족 구성원 연결(§2.9, v0.28에서 "별도 설계 결정 대기"로 남겼던 항목). 사용자 결정: admin 중개 경로(웹 콘솔/admin vs device-token 부트스트랩 두 안 중 admin 선택 — 기존 `is_admin` 인가 우회만 재사용, 새 인가모델 불필요). apps/admin `(admin)/family-members`(신규, **admin의 첫 쓰기 화면·첫 Server Action**) — 운영자가 `POST /invitations` 호출해 초대 링크 생성. apps/web `/invitations/[token]`(신규) — 초대받은 가족이 로그인 후 `POST /invitations/{token}/accept`로 수락(기존 엔드포인트 재사용, 신규 백엔드 코드 없음). 모바일 앱 변경 없음. 검증: 양쪽 앱 lint/type-check/build 통과(`/invitations/[token]`·`/family-members` 라우트 등록 확인) — 이번 세션의 브라우저 자동화가 Keycloak 로그인 제출 단계에서 반복 실패해 실 브라우저 종단검증은 다음 세션으로 이월 | NUBiz AX Initiative |
