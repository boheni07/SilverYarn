...
> **Summary**: 사용자 화면(UI/UX) 기준으로만 작성한 화면정의서 — 모바일앱(어르신 당사자)·웹(자서전 사용자·가족)·웹(관리자) 3영역, 실제 구현된 화면(2026-09-14 기준, 모바일 "사진 추가하기" 신규 반영) 전수.
>
> **Project**: 은빛실타래 (SilverYarn)
> **Version**: 0.4 (모바일 "사진 추가하기"(M-05) 신규 — decisions.md #10, report.md §7)
...
- **`GET /me` + `?elder=` 쿼리스트링 패턴(웹 사용자·가족 콘솔 전 화면 공통, ✅ 구현완료, 2026-09-13)**: `GET /me`가 로그인 세션(Keycloak 토큰)의 `AuthContext.memberships`를 그대로 돌려준다 — "로그인한 나 = 어느 어르신(들)에 연결돼 있는지"를 서버가 직접 알려준다. `apps/web/src/services/me.ts`의 `resolveCurrentUserId()`/`resolveCurrentMembership()`이 이를 소비해, 연결된 어르신이 1명이면 URL에 어떤 식별자도 없이(`/dashboard`) 자동 해석하고, 여러 명이면 W-02에서 고른 값이 `?elder=`로만 채워진다(사람이 직접 입력하지 않음). 예전 `?userId=`를 사람이 직접 입력하던 W-02(임시 홈)는 제거됐다.
- **가족 구성원 자기식별(W-09/W-10 공통, ✅ 구현완료, 2026-09-13)**: `resolveCurrentMembership()`이 `Membership.family_member_id`까지 함께 돌려주므로, "본인 것만" 수정 가능한 화면(알림 설정·대리 동의)도 이제 구성원 목록에서 "나"를 먼저 고르는 중간 단계 없이 곧장 본인 설정/동의 화면으로 들어간다.
...
flowchart TB
    subgraph MOBILE["📱 모바일앱 — 어르신 당사자(Android)"]
        ...
        M04 -. 대화로 돌아가기 .-> M03
        M03 -. "사진 추가" 진입점 .-> M05["M-05 사진 추가하기"]
        M05 -. 대화로 돌아가기 .-> M03
    end
...
> 7개에서 **4개**로 줄었다.
>
> **2026-09-14 추가**: design.md §5.1 화면 인벤토리에 이름만 있고 실제 화면이 없던
> **"사진 추가하기"(M-05)를 신규 구현**했다(report.md §7 다음 사이클 권고 1순위,
> decisions.md #10). Part A는 다시 **5개** 화면이다.
...
stateDiagram-v2
    ...
    Companion --> PhotoUpload: "📷 사진 추가" 텍스트 진입점(설치모드 무관)
    PhotoUpload --> Companion: "← 대화로 돌아가기"
```
...
| 항목 | 내용 |
|---|---|
| 파일 | `presentation/photos/PhotoUploadScreen.kt` |
| 목적 | design.md §5.1 화면 인벤토리에 이름만 있고 실제 화면이 없던 유일한 항목 — 어르신이 직접 사진을 추가한다(report.md §7 다음 사이클 권고 1순위) |
| 진입 경로 | M-03 화면 좌상단의 작은 "📷 사진 추가" 텍스트 — **설치모드 무관하게 항상 노출**(M-04 "설정"과 달리 어르신 본인이 쓰는 핵심 기능이라 일반/키오스크 구분을 두지 않았다) |
| 주요 구성요소 | "← 대화로 돌아가기" 링크, "📷 사진 찍기"/"🖼 갤러리에서 선택" 버튼 2개, 업로드 진행 상태("업로드 중…"/완료 안내/실패 시 "다시 시도") |
| 사용자 액션 | 버튼 선택 → 시스템 카메라(`ActivityResultContracts.TakePicture`) 또는 시스템 사진 선택기(`PickVisualMedia`)로 위임 → 사용자가 그 시스템 UI에서 이미 사진을 확정한 뒤 돌아오므로, 이 화면 자체의 별도 "미리보기 확인" 단계는 없이 곧장 업로드한다 |
| 다음 화면 | M-03 은실이 대화("← 대화로 돌아가기" 또는 업로드 완료 후 버튼) |
| 구현 상태 | ✅ 구현완료 |
| 비고 | decisions.md #10 확정 항목을 그대로 구현 — 업로드 전 클라이언트 측 리사이즈(장변 최대 1600px)·JPEG 80% 압축을 원본 포맷과 무관하게 항상 적용(`PhotoUploadService`). sync-contract.md §4 Presigned URL 3단계를 어르신 본인 기기(Device Token, `uploader_type=self`)로 수행 — apps/web의 가족용 업로드(W-05)와 동일한 백엔드 엔드포인트를 공유하되 인증 경로만 다르다. 캡션·연도 태그 입력 UI는 없다 — 백엔드에 애초에 그 값을 설정하는 엔드포인트 자체가 없어(웹도 동일) 만들지 않았다. CAMERA 런타임 권한은 촬영을 시스템 카메라 앱에 위임하는 방식이라 요청하지 않는다. |
...
flowchart TB
    ...
    W01 --> W02["W-02 홈<br/>(세션→어르신 자동 연결)"]
    W02 -->|"연결된 어르신 1명(자동 이동)"| W06["W-06 가족 대시보드<br/>(오늘의 기억 리포트)"]
    W02 -. "여러 명이면 선택" .-> W06
...
> `W-06`(가족 대시보드)에서 W-08~W-11로는 화면 안에 직접 내비게이션 링크가 아직 없다 —
> 각 화면은 `?elder=`만 있으면 독립적으로 동작하므로(§0.3) 직접 URL로 이동한다. 상단
> 내비게이션 바 자체는 이 기능과 무관한 별도 후속 과제로 남는다.
...
## B.2 W-02 홈 (세션→어르신 자동 연결)

| 항목 | 내용 |
|---|---|
| 파일 | `app/page.tsx` |
| 목적 | 로그인 세션(`GET /me` → `AuthContext.memberships`)만으로 "이 계정이 어느 어르신(들)에 연결돼 있는지"를 해석해 곧장 대시보드로 보낸다(§0.3 `?elder=` 패턴 참조) — 예전에 어르신 계정 ID를 사람이 직접 입력하던 임시 허브(W-02 구 버전)를 대체 |
| 주요 구성요소 | 연결된 어르신이 1명이면 화면 없이 `/dashboard?elder=<userId>`로 즉시 리다이렉트(URL에 ID가 노출되지 않음). 여러 명이면 서비스 타이틀 + 어르신 이름 목록(클릭 시 `?elder=` 채워짐). 0명(초대 미수락)이면 안내 문구만 표시 |
| 구현 상태 | ✅ 구현완료 |
| 비고 | 파일럿 스코프(decisions.md #63)상 "가족구성원 1인 : 어르신 1인"이 지배적 케이스라 실사용에서는 대부분 화면조차 보이지 않고 바로 대시보드로 넘어간다. |
...
| 주요 구성요소 | 알림 설정 폼(선택 단계 없이 바로 진입) |
| 구현 상태 | ✅ 구현완료 |
| 비고 | "본인 것만" 수정 가능 원칙(design.md §7.1)을 `resolveCurrentMembership()`(`GET /me`)이 세션에서 바로 만족시켜, 예전에 있던 구성원 선택 1단계가 사라졌다. |
...
| 주요 구성요소 | 로그인 세션 본인 명의의 동의 폼(`ConsentForm`, 선택 단계 없이 바로 진입) |
| 구현 상태 | ✅ 구현완료 |
| 비고 | decisions.md #53(2026-09-12 사용자 결정) — 성년후견 미개시 어르신에 대한 가족 대리동의를 유효로 인정한 결정의 화면 구현. `resolveCurrentMembership()`(`GET /me`)이 로그인 세션 자신의 family_member_id를 바로 알려줘 예전의 구성원 선택 1단계가 사라졌다. |
...
%%{init: {'theme':'base', 'themeVariables': {'primaryColor':'#ffffff','primaryTextColor':'#111111','primaryBorderColor':'#111111','lineColor':'#111111','background':'#ffffff','mainBkg':'#ffffff','textColor':'#111111','clusterBkg':'#ffffff','clusterBorder':'#111111','edgeLabelBackground':'#ffffff'}}}%%
flowchart TB
    A01["A-01 로그인"] --> A02["A-02 대시보드 홈"]
    A02 --> A03["A-03 사용자 관리<br/>(실질적 진입점)"]
...
|---|---|
| 파일 | `app/page.tsx` |
| 목적 | 실 Keycloak 로그인(A-01) 이후 도착하는 관리자 콘솔 허브 |
| 구성요소 | "사용자 목록 보기"/"전체 기기 통합 모니터링"/"시설 관리"/"보유기간 설정" 버튼, 보조 수단으로 "사용자 ID 직접 입력 → 기기 관리로 이동" |
| 구현 상태 | ✅ 구현완료 |
| 비고 | apps/web과 달리 관리자는 "어느 어르신에 연결됐는지" 세션 자동 해석이 필요 없다(관리자는 role 기반으로 전체 사용자를 열람 — `GET /me`/family_member 멤버십 개념의 대상이 아님). ID 직접 입력 필드는 A-03(정식 진입점)을 거치지 않고 바로 A-04로 이동하는 보조 수단으로만 남아있다. |
...
|---------|------|---------|--------|
| 0.4 | 2026-09-14 | **모바일 "사진 추가하기"(M-05) 신규**(사용자 요청, report.md §7 다음 사이클 권고 1순위) — design.md §5.1에 이름만 있고 실제 화면이 없던 유일한 항목을 구현. Part A 4개→**5개** 화면(전체 24→**25개**). decisions.md #10(카메라+갤러리, 클라이언트 리사이즈 1600px·JPEG 80%)을 그대로 구현. A.0 흐름도에 PhotoUpload 상태 추가, A.5 신설 | NUBiz AX Initiative |
| 0.3 | 2026-09-13 | **웹 실 인증 세션 연결** — `GET /me` 신규(백엔드 `AuthContext.memberships` 직렬화)로 "로그인한 나 = 어느 어르신(들)에 연결됐는지"를 세션에서 자동 해석. W-02를 어르신 ID 수동입력 임시 허브에서 **세션→어르신 자동 연결 홈**으로 교체(연결 1명이면 URL에 ID 노출 없이 즉시 대시보드 이동), 웹 사용자·가족 콘솔 9개 화면이 `?userId=` 수동 지정 대신 `?elder=`(클릭으로만 채워짐)+`resolveCurrentUserId()`로 전환. W-09/W-10(알림 설정·대리 동의)의 "구성원 먼저 선택" 1단계도 `resolveCurrentMembership()`이 `family_member_id`까지 함께 알려줘 제거. A-02는 임시방편이 아니라 정상적인 대시보드 허브였음을 재확인해 배지 정정(admin은 role 기반 전체 열람이라 세션→어르신 해석이 필요 없음). §0.3/§D/B.0·B.2·B.9·B.10·C.0·C.2 갱신 | NUBiz AX Initiative |
