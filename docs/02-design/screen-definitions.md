...
> **Summary**: 사용자 화면(UI/UX) 기준으로만 작성한 화면정의서 — 모바일앱(어르신 당사자)·웹(자서전 사용자·가족)·웹(관리자) 3영역, 실제 구현된 화면(2026-09-13 기준, PR #45까지 반영) 전수.
>
...
flowchart TB
    subgraph MOBILE["📱 모바일앱 — 어르신 당사자(Android)"]
        M01["M-01 온보딩"] --> M02["M-02 최초 동기화"] --> M03["M-03 홈"]
        M03 -->|"마이크 탭"| M04["M-04 말벗돌봄 대화"]:::mock
        M04 -->|"종료"| M03
        M03 -. 하단탭 .-> M05["M-05 자서전 작가모드"]:::off
        M03 -. 하단탭 .-> M06["M-06 비서모드"]:::off
        M03 -. 하단탭 .-> M07["M-07 설정"]:::off
    end
...
> 범례: 실선=순차 전환, 점선 화살표=시스템 간 데이터 흐름, 점선 테두리 회색 노드(`M-05`~`M-07`)=스텁, 대시 테두리 흰 노드(`M-04`)=mock 부분구현.
...
stateDiagram-v2
    ...
    Loading --> Home: device_id 있음(재실행)
    Onboarding --> FirstSync: 정보입력+동의 완료
    FirstSync --> Home: 완료 또는 건너뛰기

    state Home {
        [*] --> Idle
        Idle --> CareConversation: 마이크 탭
        CareConversation --> Idle: 종료
        Idle --> AuthorMode: 하단탭 "자서전"
        Idle --> AssistantMode: 하단탭 "일정"
        Idle --> Settings: 하단탭 "설정"
        AuthorMode --> Idle: 하단탭 "대화"
        AssistantMode --> Idle: 하단탭 "대화"
        Settings --> Idle: 하단탭 "대화"
    }
```

> 마이크 세션 진입점은 항상 "대화" 탭 안에서 발생하고, **말벗돌봄 모드로 고정 라우팅**된다 — 온디바이스 SLM 라우터(발화 의도로 자서전/말벗돌봄 자동 분류)가 아직 스텁이라, 라우터가 붙기 전까지 호출자(`AppShell`)가 목적지를 고정했다(design.md §2.11).
...
| 사용자 액션 | 실패해도 "건너뛰기"로 홈에 진입 가능(오프라인 우선 원칙) — 다음 Wi-Fi 접속 시 백그라운드 `SyncWorker`가 재시도 |
| 다음 화면 | M-03 홈 |
| 구현 상태 | ✅ 구현완료 |
...
| 목적 | 앱 실행 시 기본 진입 화면 — 인사말, 마이크 진입점, 오늘의 회고 질문, 요약 통계를 보여준다 |
| 진입 경로 | 최초 동기화(M-02) 완료 후, 또는 재실행 시 바로, 또는 대화(M-04)/다른 탭에서 복귀 |
| 주요 구성요소 | 인사말("안녕하세요, ○○님"), 원형 마이크 버튼(🎙, 96dp), 오늘의 회고 질문 카드(있을 때만), 하단 통계 3종(완성 챕터 수·연속 대화일수·미회고 사진 수) |
| 사용자 액션 | 마이크 탭 → 대화 세션 시작 / 질문 카드 탭 → (설계상)자서전 작가모드 진입이나 라우터 스텁으로 현재 미연결 |
| 다음 화면 | M-04 말벗돌봄 대화 |
| 구현 상태 | ✅ 구현완료(로컬 DB 조회 전용, 오프라인에서도 완결 동작) |
| 비고 | `stats` 로딩 전에는 통계 영역을 아예 그리지 않는다(깜빡임 방지 의도적 설계). |

## A.4 M-04 말벗돌봄 대화

| 항목 | 내용 |
|---|---|
| 파일 | `presentation/care/CareConversationScreen.kt` + `ConversationSessionController.kt` |
| 목적 | 어르신이 실제로 말벗과 대화하는 화면 — 듣기/생각/말하기 단계를 오가며 대화 로그를 쌓는다 |
| 진입 경로 | 홈(M-03)에서 마이크 탭 |
| 주요 구성요소 | 상단 바("말벗돌봄 모드" + "종료" 버튼), 대화 로그(사용자 발화·AI 응답 말풍선 목록, 좌우 정렬로 구분), 진행 단계 캡션("듣고 있어요…"/"생각하고 있어요…"/"말하고 있어요…"), 하단 마이크 버튼("🎙 말씀해 주세요") |
| 사용자 액션 | 마이크 버튼 탭 → 한 턴(듣기→생각→말하기) 진행, 처리 중엔 버튼 비활성화 / "종료" → 홈으로 복귀 |
| 다음 화면 | M-03 홈(종료 시) |
| 구현 상태 | 🟡 부분구현(mock) |
| 비고 | STT/SLM만 `MockSttEngine`(고정 문장 순환)/`MockSlmEngine`(키워드 매칭 2문장)로 대체돼 있고, 나머지 — 로컬 DB 기록(`conversations` 테이블), TTS 실 재생(`AndroidNativeTtsEngine`), 이후 `SyncWorker` 업로드 — 는 전부 실물이다. 실 마이크 캡처가 없어 "듣기"는 800ms 고정 지연으로 흉내 낸다. 실 STT/SLM 모델이 정해지면(#27, 실기기 벤치마크 대기) 두 Mock 클래스 생성부만 교체하면 된다(design.md §2.12). |

## A.5 M-05 자서전 작가모드 인터뷰 (스텁)

| 항목 | 내용 |
|---|---|
| 파일 | `presentation/author/AuthorInterviewScreen.kt` |
| 목적(원본 목업 기준) | AI 작가가 회고 질문을 던지고 답변을 자서전 챕터로 엮는 인터뷰 화면 |
| 진입 경로 | 홈의 하단탭 "자서전" |
| 주요 구성요소(현재) | "자서전 작가 모드 — 준비 중" 안내 문구 1줄뿐 |
| 구현 상태 | ⬜ 스텁 |
| 비고 | M-04와 마찬가지로 mock STT/SLM으로 구현 가능한 후보이나, 이번 작업 범위(PR #45)는 "대화 화면·세션·STT·SLM 연결"로 좁혀 말벗돌봄 모드에만 적용했다 — 다음 확장 후보. |

## A.6 M-06 비서모드 홈 (스텁)

| 항목 | 내용 |
|---|---|
| 파일 | `presentation/assistant/AssistantHomeScreen.kt` |
| 목적(원본 목업 기준) | 일정·복약 알림을 확인하고 챙기는 비서 모드 홈 |
| 진입 경로 | 홈의 하단탭 "일정" |
| 주요 구성요소(현재) | "비서 모드 — 준비 중" 안내 문구 1줄뿐 |
| 구현 상태 | ⬜ 스텁 |
| 비고 | `sync/SyncApi.kt`의 `ScheduleItemDownload`로 서버발 일정이 이미 로컬 캐시(`schedule_cache`)에 내려오고 있어 데이터는 준비돼 있다 — 화면만 미착수. |
...
| 진입 경로 | 홈의 하단탭 "설정" — **일반 모드에서만 노출**. 키오스크 모드는 Device Owner Mode(COSU) lockTask로 설정 접근 자체를 차단(decisions.md #6) |
| 주요 구성요소(현재) | "설정 — 준비 중" 안내 문구 1줄뿐 |
| 구현 상태 | ⬜ 스텁 |
| 비고 | Wi-Fi SSID는 등록되면 `device_state`에만 로컬 저장, 서버 미전송(decisions.md #22). |
...
| 모바일 M-04 | STT/SLM이 mock(고정 응답) — 실 음성인식·생성 아님 | 온디바이스 SLM 실기기 벤치마크(#27/#9/#31) 완료 후 모델 확정 |
| 모바일 M-05·M-06 | 자서전 작가모드·비서모드 화면 미착수(스텁) | 필요 시 M-04와 같은 방식(mock 엔진)으로 확장 가능 — 이번 스코프 밖 |
