...
export async function listPhotoRequests(userId: string): Promise<PhotoRequest[]> {
  return apiClient.get<PhotoRequest[]>(`/users/${userId}/photo-requests`, { authToken: "dev" });
}
...
export async function dismissPhotoRequest(requestId: string): Promise<PhotoRequest> {
  return apiClient.post<PhotoRequest>(`/photo-requests/${requestId}/dismiss`, undefined, { authToken: "dev" });
}
