...
- (같은 날 또 후속) 이름이 겹치는/안 겹치는 사용자 3명을 만들고 `/users` 검색창에
  **실제로 값을 입력하고 "검색" 버튼을 실제로 클릭**해 `?name=` 쿼리로 이동, 일치하는
  2명만 렌더링되는 것 확인 → **"지우기" 링크를 실제로 클릭**해 전체 목록으로 복귀하는
  것까지 확인했다.
...
### `(admin)/users` 이름 검색 추가 (2026-09-08)

`GET /users`에 `name` 쿼리 파라미터(부분일치, 대소문자 무시 `ILIKE`) 추가 —
care 모듈의 `conversation_chunks` 검색과 달리 "임시" 표시가 없다: 이름 문자열
매칭은 애초에 하이브리드 서치 같은 게 필요 없는 단순 조회라 `ILIKE`가 최종
구현이다. 화면은 자바스크립트 없이도 동작하는 순수 `<form method="GET">` —
검색은 새 URL로 이동하는 작업이라 클라이언트 상태가 필요 없다고 판단했다.
검색어를 바꾸면 자연히 1페이지로 리셋되고(폼에 `page` 인풋이 없음), 검색
중일 때는 "지우기" 링크로 전체 목록으로 돌아간다. 페이지네이션 링크는
`name`을 유지한 채 `page`만 바꾼다. `tests/modules/users/test_user_service.py`에
검색 테스트 4건 추가.

## 아직 안 된 것 (의도적 범위 제한)

- **실제 로그인 없음** — apps/web과 동일 이유(Keycloak 붙기 전 임시 진입점,
  Keycloak 실 연동은 인프라·인가모델 결정이 먼저 필요해 별도 작업으로 남겨 둠).
