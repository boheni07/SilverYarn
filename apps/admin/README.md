...
- **"전체 사용자 목록"이 없음** — `GET /users`(list) 엔드포인트가 아직 없어, 관리자도
  사용자 ID를 미리 알아야 기기 관리 화면에 들어갈 수 있다. 진짜 관리자 워크플로우라면
  검색/목록 화면이 필요 — 백엔드에 새 엔드포인트가 필요한 작업이라 지금 범위에서 뺐다.
...
- **사용자 관리 화면 없음** — structure.md §5 화면 인벤토리 표에는 `sync-monitor`/`devices`
  두 개만 명시돼 있고(본문 설명엔 "사용자 관리"도 언급되지만 표에 경로가 없음), 대응하는
  백엔드 목록 엔드포인트도 없어 이번 스캐폴딩에서는 만들지 않았다.
